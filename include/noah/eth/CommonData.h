...
NOAH_TX_API bool isHex(std::string const& _s) noexcept;
...
/// @example asNibbles("A")[0] == 4 && asNibbles("A")[1] == 1
    NOAH_TX_API bytes asNibbles(bytesConstRef const& _s);
...
/// @p _all if true will escape all characters, not just the unprintable ones.
    NOAH_TX_API std::string escaped(std::string const& _s, bool _all = true);
